import type {
  Request, MasterItem, SourceItem, Source, MasterSourceMap,
  MatchCandidate, DataQualityResult, ImportRun, AuditEvent,
  WorkflowHistoryEntry, Notification, Company, Department, User
} from '../types';
...
export interface MasterService {
  list(params?: { companyId?: string; search?: string; status?: string; page?: number }): Promise<{ data: MasterItem[]; total: number }>;
  getById(id: string): Promise<MasterItem | undefined>;
}
...
export interface SourceService {
  list(params?: { companyId?: string; search?: string; status?: string; page?: number }): Promise<{ data: SourceItem[]; total: number }>;
  sources: Source[];
  sourceMaps: MasterSourceMap[];
}
