...
import { PageHeader, Button, SearchInput, StatusBadge, EmptyState, Modal, Input, Textarea } from '../../components/ui';
import { WorkflowTimeline } from '../../components/workflow';
import type { AccountingCode } from '../../contracts';
...
export const AccountingList: React.FC = () => {
  ...
  if (selected) {
    const requester = users.find(u => u.id === selected.requesterId);
          ...
          <div className="stack">
            <div className="card p16">
              ...
              <div className="review-grid" style={{ marginTop: 8 }}>
                ...
                <div><span className="muted small">Grupo</span><br /><strong>{findName(groups, selected.groupId)}</strong></div>
                <div><span className="muted small">Subgrupo</span><br /><strong>{findName(subgroups, selected.subgroupId)}</strong></div>
                <div><span className="muted small">Categoría</span><br /><strong>{findName(categories, selected.categoryId)}</strong></div>
                <div><span className="muted small">Marca</span><br /><strong>{findName(brands, selected.brandId)}</strong></div>
