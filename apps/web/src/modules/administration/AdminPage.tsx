...
import { companies, departments, users, roles } from '../../mock/companies';
      ...
      {tab === 0 && (
        <Section title="Usuarios">
          <table className="table">
            <thead><tr><th>Nombre</th><th>Usuario</th><th>Email</th><th>Roles</th><th>Estado</th></tr></thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id}>
            ...
            <tbody>
              {departments.map(d => (
                <tr key={d.id}>
                  ...
                  <td>{companies.find(c => c.id === d.companyId)?.name || '—'}</td>
                  <td>{users.find(u => u.id === d.managerId)?.displayName || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
