...
import { users } from '../../mock/companies';
import { PageHeader, SearchInput, Badge, EmptyState, Modal, Button } from '../../components/ui';
            ...
            <tbody>
              {filtered.map(ev => (
                <tr key={ev.id} className="clickable" onClick={() => setSelected(ev)}>
                  ...
                  <td>{users.find(u => u.id === ev.actorId)?.displayName || ev.actorId}</td>
                  <td>{ev.entityType}</td>
        ...
        {selected && (
          <div className="stack-sm">
            <div className="review-grid">
              ...
              <div><span className="muted small">Usuario</span><br />{users.find(u => u.id === selected.actorId)?.displayName || selected.actorId}</div>
