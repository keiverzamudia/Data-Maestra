...
export interface WorkflowTask { id: string; requestId: string; stepCode: RequestStatus; assigneeId?: string; status: 'PENDING'|'CLAIMED'|'COMPLETED'|'RETURNED'; dueAt?: string; }
...
export interface ItemAlias { id: string; masterItemId: string; alias: string; normalizedAlias: string; }
...
export interface MasterSourceMap { id: string; masterItemId: string; sourceItemId: string; relationType: string; confidence?: number; active: boolean; }
...
export interface ImportRun { id: string; sourceId: string; startedAt: string; finishedAt?: string; status: ImportStatus; rowsRead: number; rowsImported: number; rowsUnchanged: number; rowsFailed: number; rowsSkipped: number; }
...
export interface DashboardStats {
  pendingRequests: number; inApproval: number; returnedRequests: number; completedRequests: number;
  activeMasterItems: number; pendingHomologation: number; pendingSourceItems: number; pendingMatches: number;
  qualityIssues: number; recentImports: number;
}
